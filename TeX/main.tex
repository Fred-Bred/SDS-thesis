...
This work aims to determine whether tested methods in the field of Natural Language Processing (NLP) can begin to solve this complex task with some proficiency.
As such, the aim is not to develop on the technical solutions available for this type of task, nor to produce major advancements in theory or clinical practice.
...
As such it is fundamentally a BERT model and shares most of its architecture with its 2018 ancestor develop by Sun et al. \citeyear{Sun2020}.
...
This server directly accesses the password protected secure drive storing the data.
...
Figure \ref{fig: class balance turn len} reveals that, in fact, the class balance in the short speech turns varies substantially from the distribution in the full dataset.
...
To investigate whether models would benefit from seeing only longer sequence of patient speech I conducted preprocessing experiments in which I defined a minimum speech turn length, measured in word count, and combined speech turns within documents until the minimum count had been reached.
...
In the evaluation of classification models, two baselines for comparison are common: The random baseline and the majority baseline, i.e., the accuracy achieved by a model which consistently predicts the majority label.
...
On three-way classification in the original validation study, PACS raters were in excellent agreement with $\kappa = 0.87$ \cite{Talia2017}.
In comparison, the mean $\kappa$ in classifying the test samples for the RoBERTa-large models trained with minimum input length of 150 words was just 0.15 (see Table \ref{tab:roberta-large_150 test metrics}).
As any value above 0 indicates greater performance than would be expected by chance, even this low value provides an initial indication that the models have learned meaningful representations for the classification task.
