...
The second approach is impact evaluation, which relates mostly to the impact of generated text, making it less applicable here.
Nevertheless, the impact of the future application of an automatic assessment tool in research and practice should be continually evaluated.
...
Each accuracy score is reported in table \ref{tab:roberta-large_150 test acc} in Appendix \ref{App: confusion matrices}.
It is noteworthy that the accuracy score of the ensemble voting classifier using all five instances of the RoBERTa-large model results in an accuracy of 67.42 \%.
...
% Discuss results.
% - Do we beat the random baseline?
% - Do we beat the majority baseline?
...
Due to the bias for anxious classification in most models, nn the ensemble classification, the mislabelled avoidant patients are instead classified as anxious.
...
Rather, in the test split, concatenating to at least 150 words results in an increased skew:
\begin{itemize}
    \item Avoidant: 24.72 \%
    \item Secure: 13.48 \%
    \item Anxious: 61.80 \%
...
This skewed distribution challenges the interpretation of the above results as demonstrating the begging of learning to distinguish classes.
In fact, the majority of models do not perform above the majority baseline.
...
\begin{table}
    ...
    \end{adjustbox}
    \caption{Test Set Accuracies for Each Instance of the RoBERTa-large Model Trained with Minimum Input Length of 150 Words.}
    \label{tab:roberta-large_150 test acc}
\end{table}
