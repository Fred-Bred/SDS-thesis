...
As norms differ between therapeutic traditions and the countries where the sessions studied in this thesis were held, some terminology may be used interchangeably in this manuscript.
...
As such, performance above the random baseline would indicate that there are generalisable patterns relevant to each of the three main attachment classifications and that these patterns can be picked up as signal by sufficiently complicated language models.
...
The transformer is a unique deep learning method originally demonstrated by Vaswani et al. \citeyear{Vaswani2017} for NLP purposes.
...
\end{itemize}
The majority class in this dataset is avoidant.
As noted, it was shown by Feeney and Ryan \citeyear{Feeney1994} that those with avoidant attachment styles were more prone to seeking medical attention and more likely to report having had therapy.
However, considering the review of 'the first 10,000 adult attachment interviews' by Bakermans-Kranenburg and IJzendoorn \citeyear{Bakermanskranenburg2009} which found that in clinical populations, 23 \% were avoidant, 21 \% secure, and 13 \% anxious, and 43 \% were unresolved, this is somewhat surprising.
...
In this continuum, a person can move away from feelings of security in either direction: Toward avoidance, becoming dismissive of attachments, or toward anxiety, becoming preoccupied with attachments.
...
As reviewed in section \ref{sec:theory and review}, a large part of variation in outcomes is explained by the binary classification of secure and insecure attachment alone, and the central insight likely lies in differentiating anxious and avoidant patients to inform the alliance building and treatment process.
...
Nevertheless, it is important to split at the document level to avoid data leakage and ensure that the model is not learning to recognise individuals but rather generalisable speech patterns according to PACS classification.
...
All models were implemented using the Massive Choice, Ample Tasks (MaChAmp) framework for transfer- and multi-task learning developed by van der Goot et al. \citeyear{MaChAmp}.
...
This resulted in a total of 26,083 utterances.
...
Each model trained on this data for 20 epochs, seeing each sample once, and evaluating perplexity on the development set after each epoch.
...
Following experiments, a starting learning rate of 1e-5 was selected.
While a learning rate of 5e-5 was found by Sun et al. \citeyear{Sun2020} to work well for text classification tasks with similar types of pre-trained models, experiments with this higher starting learning rates produced unstable training while lower learning rates generally produced poorer metrics, likely as models would be unable to escape local minima.
A slanted triangular learning rate schedule is implemented according to the formulation by Howard and Ruder \citeyear{Howard2018}.
This schedule implements a linear warm-up for a specified fraction of the training steps, here, 0.3, before linearly decreasing the learning rate.
* Multiple annealing cycles
* Ratio??

\paragraph*{Optimisation and Regularisation}
Optimisation is performed using the AdamW introduced by Loshchilov and Hutter \citeyear{AdamW}.
While both the first and second moments are calculated as in the original adam implementation, AdamW implements weight decay in the parameter update step.

Upon updating the model's parameters, weight decay applies a decay factor to the parameters at the previous step.
This ensures that all weights are gradually decreased unless explicitly updated, effectively regularising the model.
The 
...
See Figure \ref{fig: acc over turn len} in Appendix \ref{App: Figures} for an example of a plot of one such experiment.
...
The dichotomy of machine and clinical judgement does not have to be. In fact, a union is not only legitimate but justified, given adequate performance.
...
In the case of using language models in psychological research and practice, Demszky et al. \citeyear{Demszky2023} point to two primary sources or types of harms arising from bias.
...
Cultural variations in how attachment styles are manifested might still exist, but these findings support the potential applicability of machine learning approaches to attachment assessment across cultures.
...
\begin{figure}
    \includegraphics[width=\textwidth]{figures/full_turn_length_distribution.png}
    \caption{Length of patient speech turns in the PACS transcripts}
    \label{fig: PACS turn length}
\end{figure}


\begin{figure}
    \includegraphics[width=\textwidth]{figures/metrics_by_length_2024.03.13_11.54.44_model 7 2.png}
    \caption{Example of accuracy over turn length for one model experiment}
    \label{fig: acc over turn len}
\end{figure}

\begin{figure}
    \includegraphics[width=\textwidth]{figures/turn_length_distribution_by_class.png}
