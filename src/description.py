...
# Load patient speech turns from all documents in folder
daic_woz = pd.load_csv(os.join(DAIC_WOZ_path, daic_woz_files[0]))
for file in daic_woz_files[1:]:
    df = pd.load_csv(os.join(DAIC_WOZ_path, file))
...
# Load patient speech turns from all documents in folder
hope = pd.load_csv(os.join(HOPE_path, hope_files[0]))
for file in hope_files[1:]:
    df = pd.load_csv(os.join(HOPE_path, file))
    hope = hope.concat(df)
