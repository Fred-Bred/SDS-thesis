...
for i in range(1, 6):
    ...
    with open(f"{output_folder}/split{i}_retrained_{mode}_metrics.txt", "w") as f:
...
best_metrics_txt = f"{output_folder}/split{best_model}_retrained_{mode}_metrics.txt"
with open(best_metrics_txt, "r") as f:
    best_metrics = f.read()

# Save average accuracy to file
with open(f"{output_folder}/{mode}_retrained_average_accuracy.txt", "w") as f:
