    ...
    if [ -d "$dir" ]; then
        ...
        for file in "$dir"/*; do
            ...
            if [[ $file == *"$search_string"* ]]; then
                ...
                output_dir="../Outputs/trained_models/k-folds/MentalRoBERTa_150"
                
                # Create the output directory if it doesn't exist
                mkdir -p "$output_dir"
                
                python3 predict.py "$file" "../Data/k-folds/split${split_num}/val_150.csv" "$output_dir/split${split_num}_val_preds.csv"
                break
            fi
        done
    fi
